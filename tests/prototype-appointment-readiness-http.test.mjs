...
async function signIn(email) {
  const response = await fetchWithTransientRetry(`${apiUrl}/auth/v1/token?grant_type=password`, {
    method: "POST",
...
test.before(async () => {
  ...
  const invocation = process.platform === "win32"
    ? { command: "powershell.exe", args: ["-NoProfile", "-NonInteractive", "-Command", `& '${supabaseCommand}' functions serve prototype-appointment-readiness --env-file '${envFile}' --no-verify-jwt`] }
    : { command: supabaseCommand, args: ["functions", "serve", "prototype-appointment-readiness", "--env-file", envFile, "--no-verify-jwt"] };
  ...
  const functionUrl = `${apiUrl}/functions/v1/prototype-appointment-readiness`;
  ...
  const createUser = (email, appMetadata = {}) => fetchWithTransientRetry(`${apiUrl}/auth/v1/admin/users`, {
    method: "POST",
  ...
  Object.assign(scenario, {
    ...
    bookingAttemptUrl: `${apiUrl}/functions/v1/prototype-appointment-attempt`,
    tokens: {
...
test("one Business becomes active only after every sandbox pilot check passes", { skip: !runHttpTests }, async () => {
  ...
  assert.equal((await independent.json()).readiness.status, "disabled");
});
