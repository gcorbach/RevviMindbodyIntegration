...
test("availability page covers live times, empty dates, and review responsively", { skip: !chromePath || process.env.RUN_BROWSER_TESTS !== "1" }, async (testContext) => {
  ...
  const resolvingHtml = source.replace("<script>", '<script>window.REVVI_CUSTOMER_TOKEN = "test-token"; window.REVVI_CATALOGUE_API_URL = "/functions/v1/booking-availability"; window.REVVI_BOOKING_API_URL = "/functions/v1/booking-attempt?state=unknown"; window.REVVI_TEST_RESPONSIVE = true;\n').replace("</body>", '<script>const testClick = setInterval(() => { const button = document.querySelector("#continue-booking"); if (button && !button.disabled && !button.closest("[hidden]")) { clearInterval(testClick); button.click(); } }, 25);</script></body>');
  const failedHtml = source.replace("<script>", '<script>window.REVVI_CUSTOMER_TOKEN = "test-token"; window.REVVI_CATALOGUE_API_URL = "/functions/v1/booking-availability"; window.REVVI_BOOKING_API_URL = "/functions/v1/booking-attempt?state=failed"; window.REVVI_TEST_RESPONSIVE = true;\n').replace("</body>", '<script>const testClick = setInterval(() => { const button = document.querySelector("#continue-booking"); if (button && !button.disabled && !button.closest("[hidden]")) { clearInterval(testClick); button.click(); } }, 25);</script></body>');
  const expiredHtml = source.replace("<script>", '<script>window.REVVI_CUSTOMER_TOKEN = "test-token"; window.REVVI_CATALOGUE_API_URL = "/functions/v1/booking-availability"; window.REVVI_BOOKING_API_URL = "/functions/v1/booking-attempt?state=expired"; window.REVVI_TEST_RESPONSIVE = true;\n').replace("</body>", '<script>const testClick = setInterval(() => { const button = document.querySelector("#continue-booking"); if (button && !button.disabled && !button.closest("[hidden]")) { clearInterval(testClick); button.click(); } }, 25);</script></body>');
  const staleHtml = source.replace("<script>", '<script>window.REVVI_CUSTOMER_TOKEN = "test-token"; window.REVVI_CATALOGUE_API_URL = "/functions/v1/booking-availability"; window.REVVI_BOOKING_API_URL = "/functions/v1/booking-attempt?stale=true"; window.REVVI_TEST_RESPONSIVE = true;\n').replace("</body>", '<script>let recovered = false; let initialSubmitted = false; const testClick = setInterval(() => { const slot = document.querySelector("#slots button"); if (!recovered && slot && !slot.closest("[hidden]")) { recovered = true; slot.click(); return; } const button = document.querySelector("#continue-booking"); if (button && !button.disabled && !button.closest("[hidden]")) { if (!initialSubmitted) { initialSubmitted = true; button.click(); return; } clearInterval(testClick); button.click(); } }, 25);</script></body>');
  const server = createServer((request, response) => {
    if (request.url?.startsWith("/functions/v1/booking-attempt")) {
      ...
      if (bookingState) { response.writeHead(bookingState === "unknown" ? 202 : 409, { "content-type": "application/json" }); response.end(JSON.stringify({ code: bookingState === "unknown" ? "BOOKING_RESOLVING" : bookingState === "expired" ? "BOOKING_ATTEMPT_EXPIRED" : "BOOKING_NOT_COMPLETED", error: "Booking status test response.", bookingAttempt: { state: bookingState } })); return; }
  ...
  try {
    ...
    assert.match(resolving.stdout, /Resolving your Booking/); assert.match(resolving.stdout, /Do not submit it again/);
