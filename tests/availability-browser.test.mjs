...
test("availability page covers live times, empty dates, and review responsively", { skip: !chromePath || process.env.RUN_BROWSER_TESTS !== "1" }, async (testContext) => {
  ...
  const server = createServer((request, response) => {
    if (request.url?.startsWith("/functions/v1/booking-attempt")) {
      response.writeHead(200, { "content-type": "application/json" }); response.end(JSON.stringify({ bookingAttempt: { state: "confirmed", service: "Nutrition Consultation", startTime: "2026-07-28T16:00:00.000Z", locationTimezone: "America/Los_Angeles" } })); return;
    }
  ...
  const launch = async (query, size = "390,844") => {
    ...
    try {
      return await new Promise((resolve) => {
        const child = spawn(chromePath, ["--headless=new", "--disable-gpu", "--disable-extensions", "--no-first-run", "--no-default-browser-check", `--window-size=${size}`, "--dump-dom", "--virtual-time-budget=1500", `--user-data-dir=${profile}`, `http://127.0.0.1:${port}/?business=sandbox-wellness&location=sandbox-location&service=service-23&${query}`], { windowsHide: true });
        let stdout = "";
        let stderr = "";
        let settled = false;
        const finish = (result) => {
