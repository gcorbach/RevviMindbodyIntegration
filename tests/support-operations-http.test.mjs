...
test.before(async () => {
  ...
  const invocation = process.platform === "win32"
    ? { command: "powershell.exe", args: ["-NoProfile", "-NonInteractive", "-Command", `& '${supabaseCommand}' functions serve support-operations --env-file '${envFile}' --no-verify-jwt`] }
    : { command: supabaseCommand, args: ["functions", "serve", "support-operations", "--env-file", envFile, "--no-verify-jwt"] };
  ...
  const functionUrl = `${apiUrl}/functions/v1/support-operations`;
