...
test("HTTP availability and review use tenant policy and a controllable Mindbody double", { skip: !runHttpTests }, async () => {
  ...
  const invocation = process.platform === "win32"
    ? { command: "powershell.exe", args: ["-NoProfile", "-NonInteractive", "-Command", `& '${supabaseCommand}' functions serve prototype-appointment-availability --env-file '${envFile}' --no-verify-jwt`] }
    : { command: supabaseCommand, args: ["functions", "serve", "prototype-appointment-availability", "--env-file", envFile, "--no-verify-jwt"] };
  ...
  try {
  const functionUrl = `${localSupabase.API_URL}/functions/v1/prototype-appointment-availability`;
