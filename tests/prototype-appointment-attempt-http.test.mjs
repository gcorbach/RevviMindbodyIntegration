...
const appointmentPrototypeBusinessIds = [
  "00000000-0000-0000-0000-000000000011",
  "00000000-0000-0000-0000-000000000013",
  "00000000-0000-0000-0000-000000000014",
];

function setAppointmentPrototypeReadiness(status) {
  const ids = appointmentPrototypeBusinessIds.map((id) => `'${id}'`).join(", ");
  const activeValues = status === "active"
    ? "status = 'active', activated_at = now(), deactivated_at = null, deactivation_reason = null"
    : "status = 'disabled', activated_at = null, deactivated_at = now(), deactivation_reason = 'appointment_prototype_only'";
  const sql = `update public.business_pilot_readiness set ${activeValues} where business_id in (${ids})`;
  const update = spawnSync("docker", ["exec", "supabase_db_revvi-booking", "psql", "-U", "postgres", "-d", "postgres", "-v", "ON_ERROR_STOP=1", "-c", sql], { encoding: "utf8", windowsHide: true });
  assert.equal(update.status, 0, update.stderr);
}

test.before(() => {
  if (runHttpTests) setAppointmentPrototypeReadiness("active");
});

test.after(() => {
  if (runHttpTests) setAppointmentPrototypeReadiness("disabled");
});
...
async function startBookingScenario({ memberstackId, clientMode = "existing", extraEnvironment = [], functionName = "prototype-appointment-attempt", startTime = nextTestDoubleSlot(), resetFixtures = true }) {
  ...
  const functionEnvironmentEntries = [
    "MINDBODY_API_KEY=stub-key", "MINDBODY_SANDBOX_SITE_ID=stub-site", "MINDBODY_BASE_URL=http://test-double.invalid/public/v6", "MINDBODY_ENVIRONMENT=sandbox", "MINDBODY_ALLOW_TEST_DOUBLE=true", "REVVI_APPOINTMENT_PROTOTYPE_ENABLED=true", `MINDBODY_TEST_DOUBLE_CLIENT_MODE=${clientMode}`, "MINDBODY_TEST_DOUBLE_EMPTY_DATE=2026-07-29", ...extraEnvironment,
  ];
...
test("HTTP booking attempt completes an existing-client free Booking idempotently", { skip: !runHttpTests }, async () => {
  ...
  writeFileSync(envFile, [
    "MINDBODY_API_KEY=stub-key", "MINDBODY_SANDBOX_SITE_ID=stub-site", "MINDBODY_BASE_URL=http://test-double.invalid/public/v6", "MINDBODY_ENVIRONMENT=sandbox", "MINDBODY_ALLOW_TEST_DOUBLE=true", "REVVI_APPOINTMENT_PROTOTYPE_ENABLED=true", "MINDBODY_TEST_DOUBLE_EMPTY_DATE=2026-07-29",
  ].join("\n"));
  const invocation = process.platform === "win32"
    ? { command: "powershell.exe", args: ["-NoProfile", "-NonInteractive", "-Command", `& '${supabaseCommand}' functions serve prototype-appointment-attempt --env-file '${envFile}' --no-verify-jwt`] }
    : { command: supabaseCommand, args: ["functions", "serve", "prototype-appointment-attempt", "--env-file", envFile, "--no-verify-jwt"] };
  ...
  const diagnostics = []; functionProcess.stdout.on("data", (chunk) => diagnostics.push(chunk.toString())); functionProcess.stderr.on("data", (chunk) => diagnostics.push(chunk.toString()));
  try {
  const functionUrl = `${apiUrl}/functions/v1/prototype-appointment-attempt`;
    await waitForFunction(functionUrl, functionProcess, diagnostics);
...
test("HTTP booking attempt retains its Client-resolution lock when mapping a created Client conflicts", { skip: !runHttpTests }, async () => {
  ...
  try {
    ...
    const cleanupOwner = await fetch(`${apiUrl}/rest/v1/mindbody_client_mappings?business_id=eq.00000000-0000-0000-0000-000000000011&memberstack_id=eq.issue-14-existing-provider-owner`, {
      method: "DELETE",
      headers: { ...scenario.adminHeaders, Prefer: "return=minimal" },
    });
    assert.ok([200, 204].includes(cleanupOwner.status), await cleanupOwner.text());
...
test("HTTP callback records correlation for an expired attempt without reviving it", { skip: !runHttpTests }, async () => {
  ...
  try {
    ...
    functionName: "prototype-appointment-callback",
...
test("HTTP approved Mindbody Checkout confirms only after checkout succeeds", { skip: !runHttpTests }, async () => {
  const scenario = await startBookingScenario({
    ...
    clientMode: "existing",
    extraEnvironment: ["BOOKING_SCA_CALLBACK_URL=http://127.0.0.1:54321/functions/v1/prototype-appointment-callback"],
  });
...
test("HTTP SCA resume reuses one Booking attempt and preserves transaction IDs", { skip: !runHttpTests }, async () => {
  const scenario = await startBookingScenario({
    ...
    clientMode: "existing",
    extraEnvironment: [
    "BOOKING_SCA_CALLBACK_URL=http://127.0.0.1:54321/functions/v1/prototype-appointment-callback",
...
test("HTTP SCA callback returns to Revvi without confirming the Booking", { skip: !runHttpTests }, async () => {
  ...
  try {
    ...
    const [attempt] = await attempts.json();
    await booking.stop();
  callback = await startBookingScenario({ memberstackId: "issue-17-checkout-callback", functionName: "prototype-appointment-callback", extraEnvironment: ["BOOKING_SCA_RETURN_URL=https://booking.example.test/availability.html"], resetFixtures: false });
...
test("HTTP checkout failure does not confirm the Booking", { skip: !runHttpTests }, async () => {
  const scenario = await startBookingScenario({
    ...
    clientMode: "existing",
    extraEnvironment: ["BOOKING_SCA_CALLBACK_URL=http://127.0.0.1:54321/functions/v1/prototype-appointment-callback", "MINDBODY_TEST_DOUBLE_CHECKOUT_OUTCOME=failed"],
  });
  const request = { ...scenario.request, business: "sandbox-checkout", location: "checkout-location", service: "00000000-0000-0000-0000-000000000033" };
  try {
    const response = await fetch(scenario.functionUrl, { method: "POST", headers: scenario.headers, body: JSON.stringify(request) });
...
test("HTTP unknown checkout outcome blocks a second checkout write", { skip: !runHttpTests }, async () => {
  const scenario = await startBookingScenario({
    ...
    clientMode: "existing",
    extraEnvironment: ["BOOKING_SCA_CALLBACK_URL=http://127.0.0.1:54321/functions/v1/prototype-appointment-callback", "MINDBODY_TEST_DOUBLE_CHECKOUT_OUTCOME=unknown"],
  });
  const request = { ...scenario.request, business: "sandbox-checkout", location: "checkout-location", service: "00000000-0000-0000-0000-000000000033" };
  try {
    const first = await fetch(scenario.functionUrl, { method: "POST", headers: scenario.headers, body: JSON.stringify(request) });
