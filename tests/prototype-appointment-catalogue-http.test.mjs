...
test("HTTP catalogue acceptance uses local Postgres/RLS and a controllable Mindbody double", { skip: !runHttpTests }, async () => {
  ...
  const functionInvocation = process.platform === "win32"
    ? {
        ...
        args: ["-NoProfile", "-NonInteractive", "-Command", `& '${supabaseCommand}' functions serve prototype-appointment-catalogue --env-file '${envFile}' --no-verify-jwt`],
        shell: false,
      }
      : cliInvocation(["functions", "serve", "prototype-appointment-catalogue", "--env-file", envFile, "--no-verify-jwt"]);
  ...
  try {
  const functionUrl = `${localSupabase.API_URL}/functions/v1/prototype-appointment-catalogue`;
