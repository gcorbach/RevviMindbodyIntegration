...
test("staff operations page covers queue, detail, and recorded resolution", { skip: !chromePath || process.env.RUN_BROWSER_TESTS !== "1" }, async (testContext) => {
  ...
  `;
  const html = source
    .replace("<script>", '<script>window.REVVI_STAFF_TOKEN = "test-staff-token"; window.REVVI_SUPPORT_API_URL = "/functions/v1/prototype-appointment-support"; window.REVVI_TEST_RESPONSIVE = true;\n')
  ...
  const server = createServer(async (request, response) => {
      if (request.url?.startsWith("/functions/v1/prototype-appointment-support")) {
