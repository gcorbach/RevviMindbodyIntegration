...
Authorization: Bearer <Supabase access token with app_metadata.identity_provider=memberstack and app_metadata.memberstack_id>
```
...
Run the local database/RLS suite with `pnpm supabase test db`. On non-Windows runners, the opt-in HTTP acceptance test uses a local Mindbody double after the local Supabase stack is up:
