...
A partner Business can configure the commercial and scheduling ingredients that Revvi needs: an activated Mindbody Site, a selected Location, an online-visible scheduled Class, and a dedicated pricing option or other explicit entitlement arrangement. Revvi can then discover Site, Location, class taxonomy, Class occurrence, pricing-option, client-pass, roster, Visit, sale, and transaction identifiers through Mindbody APIs.

The safest pilot configuration is **one dedicated pricing option for one Revvi Offer at one Location**, assigned only to the intended class inventory. Revvi should store `Services[].ProductId` as the commercial key and `Class.Id` as the occurrence key. It must not use a displayed name, `Service.Id`, or barcode as a substitute. Existing client entitlements must be resolved to an exact client-service/pass record before booking.
