...
Memberstack recommends caching the JWKS in memory; its Node example uses a 24-hour cache and up to five keys. A standards-based JWT/JWKS library should perform algorithm, signature, issuer, audience, and expiry checks atomically. Refresh the JWKS once when the selected `kid` is absent. Hosted test-mode evidence showed that Memberstack's test JWT key was still absent after refresh, so Revvi then uses the official Admin REST verification endpoint with the test-mode server key and revalidates the returned identity, type, issuer, audience, and expiry. A known JWKS key with an invalid signature never falls through to remote verification. [Memberstack custom-backend security guide](https://docs.memberstack.com/hc/en-us/articles/7253255689755-Using-Permissions-and-Token-Verification-to-Secure-Your-Site-on-a-Custom-Backend), [Admin REST verification](https://developers.memberstack.com/admin-rest-api/verification)
